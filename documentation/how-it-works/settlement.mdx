...
flowchart TD
    ...
    B[lock both unlinkable input-use tags before their expiry]
    C[verify one batch proof on-chain]
    A --> B --> C
    C --> M1["send match 1 independently → confirmed / rejected / ambiguous"]
    C --> M2["send match 2 independently → confirmed / rejected / ambiguous"]
    C --> M3["..."]
    M1 --> R[marker reclaimed only after its expiry]
    M2 --> R
    M3 --> R
```
...
[Privacy & Attestation](./privacy-and-attestation.mdx).

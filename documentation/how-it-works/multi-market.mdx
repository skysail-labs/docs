...
```mermaid
flowchart TD
    A["one verified origin + one CVM + one /v1/stream session"]
    A --> B["SOL-USDC<br/>own book"]
    A --> C["BTC-USDC<br/>own book"]
    A --> D["another pair<br/>own book"]
```
...
[Privacy & Attestation](./privacy-and-attestation.mdx) for the trust check, and
[Settlement](./settlement.mdx) for the one-market-per-proof boundary.

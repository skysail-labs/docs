...
```mermaid
flowchart TD
    Wallet[Wallet] -->|"deposit: funds → note"| Vault["Vault (L1)<br/>custody + note tree"]
    Vault -->|"note exists in tree"| Client["Client / SDK<br/>build note, proof, order"]
    Client -->|"signed order + input proof"| Enclave["Enclave (matching)<br/>batch auction, prove matches"]
...
| **3. Submit** | Client → Enclave | The signed order is sent over RA-TLS that terminates inside the enclave; the client verifies the live certificate against a fresh transport quote before sending it ([Transport & Attestation](/api-reference/getting-started/transport-and-attestation)). The enclave routes its signed symbol to one market, then verifies the signature, boot session, strictly increasing per-key nonce, viewing point, note opening, recent proof root, and input proof. It never touches a Solana transaction. |
| **4. Match** | Enclave | Each market book independently collects crossing orders and clears them at a single **oracle-anchored price** (see [Clearing Price](../trading-concepts/clearing-price.mdx)). Note-bound owner identity plus trading-key equality prevents accidental self-matches. |
...
See [Privacy & Attestation](./privacy-and-attestation.mdx) for how you verify the
enclave is the real engine, and [Settlement](./settlement.mdx) for the on-chain
pipeline in detail.

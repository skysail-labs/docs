...
```mermaid
flowchart TD
    A[use note] --> B[publish note-use tag]
    B --> C[try to use it again]
    C --> D["same consumed-note PDA already exists → rejected"]
```
...
```mermaid
flowchart TD
    A[deposit] --> B["note appended to the tree (SPENDABLE)"]
    B --> C["place order → note reserved inside the venue while resting"]
    C --> D["private match → on-chain note lock created (PENDING SETTLEMENT)"]
    D --> E["settle → input note-use tag marked consumed; outputs appended:<br/>• your filled asset (a new note)<br/>• a change note for any unfilled remainder<br/>• fee notes"]
    E --> F["withdraw → note-use tag consumed; tokens released to your wallet"]
```
...
twice regardless of what the engine does. See [Deposit](../account/deposit.mdx) and
[Withdraw](../account/withdraw.mdx) for the on-ramp and off-ramp,
[Account Model](../account/account-model.mdx) for how you reconstruct your spendable
set, and [Settlement](./settlement.mdx) for the on-chain spend pipeline.

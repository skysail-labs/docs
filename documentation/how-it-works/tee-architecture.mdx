...
```text
Intel TDX quote + measured event log
               │
               ▼
client verifies the approved image and complete signer set
               │
               ▼
finalized VaultConfig contains that same ordered signer set
               │
               ▼
Solana accepts a settlement only with a registered signature
and a valid Groth16 proof
```
...
reconciliation continue. Authenticated-oracle staleness or replay failure
pauses only markets bound to the affected feed; each `/instruments` entry
...
another. See
[Multi-Market Venue](./multi-market.mdx).
...
See [Privacy & Attestation](./privacy-and-attestation.mdx) for the verification chain
and [Settlement](./settlement.mdx) for the proof boundary.

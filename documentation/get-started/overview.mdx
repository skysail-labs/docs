...
enforced by a proof the chain verifies. See [Why Darknyx](./design-thesis.mdx) for the
...
```mermaid
flowchart BT
    Client["CLIENT — your wallet + the SDK<br/>Builds notes, generates the input proofs, signs orders with a trading key, and verifies the enclave's attestation."]
    Matching["MATCHING — the in-enclave engine (the CVM)<br/>Takes hidden orders, runs a uniform-clearing-price batch auction, and drives the settlement transactions directly. Never sees your spending key; cannot move funds itself."]
    Custody["CUSTODY — the Solana vault program<br/>Holds funds. Owns the Merkle tree of note commitments, replay guards, and the Groth16 verifier. The only layer that can move pooled tokens, subject to protocol rules."]
    Client -->|"signed orders + ZK input proofs over HTTPS/WS"| Matching
    Matching -->|"attested, signed settle txs + ZK proofs"| Custody
```
...
- [Why Darknyx](./design-thesis.mdx): the product thesis, exact trust boundary, and
  tradeoffs.
- [Programmatic Access](./programmatic-access.mdx): the API surface, the auth
  model, and a quick start.
- [Base URLs](/api-reference/getting-started/base-urls): where the endpoints live and the common
  response conventions.
- [Trade Flow](../how-it-works/trade-flow.mdx): the end-to-end lifecycle of an
  order, from submission to on-chain settlement.
- [Multi-Market Venue](../how-it-works/multi-market.mdx): how several isolated

...
- **An attested confidential VM matches private orders.** Its code identity and
  ...
  can leave, and a new engine boot must establish and verify a fresh transport
  identity.
...
| Counterparty-selection fairness | Attested matcher code | Selection among otherwise compatible counterparties is matcher behavior, not a property re-proven by the settlement circuit. |
...
- [Trade Flow](../how-it-works/trade-flow.mdx) follows one order end to end.
- [Privacy & Attestation](../how-it-works/privacy-and-attestation.mdx) explains how
  clients verify the confidential matcher.
- [Programmatic Access](./programmatic-access.mdx) maps the integration surface.

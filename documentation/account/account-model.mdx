...
```mermaid
flowchart TD
    A["your spending key + the public Merkle tree of note commitments"]
    B["the notes you own → their amounts → your spendable balance"]
    C["which are unspent (no path-specific consume record) → what you can use"]
    A --> B --> C
```
...
| You want | Read | Page |
|---|---|---|
| The current state of the on-chain tree | `GET /tree/root` | [Merkle Proofs](/api-reference/tree) |
| An inclusion proof for one of your notes | `GET /tree/inclusion` | [Merkle Proofs](/api-reference/tree) |
| A page of raw leaves (to rebuild a local mirror) | `GET /tree/leaves` | [Merkle Proofs](/api-reference/tree) |
| Your open orders | `GET /account` (all), or `GET /orders/{order_id}` + the orders stream | [Get Order](/api-reference/orders/get-order), [Orders Channel](/api-reference/websocket/orders-channel) |
| Your continuation fills | the fills stream / your durable history | [Fills Channel](/api-reference/websocket/fills-channel) |
| Venue-wide solvency | `GET /transparency` | [Transparency](/api-reference/settlement/transparency) |
...
[SDK → TypeScript Client](../sdk/typescript-client.mdx).
...
```mermaid
stateDiagram-v2
    state "VENUE-RESERVED" as VenueReserved
    state "ON-CHAIN LOCKED" as OnChainLocked
    [*] --> SPENDABLE: deposit
    SPENDABLE --> CONSUMED: merge / withdraw
    SPENDABLE --> VenueReserved: accepted order
    VenueReserved --> OnChainLocked: matched
    OnChainLocked --> CONSUMED: settle
    CONSUMED --> [*]
```
...
|---|---|---|
| `cancel_on_disconnect_default` | `false` | When `true`, a [trading socket](/api-reference/websocket/ws-trading) or [session stream](/api-reference/websocket/session-stream) for this account cancels your open orders when it disconnects. A socket's own `cancel_on_disconnect` overrides this default. |

...
|---|---|---|
| [Place Order](place-order.mdx) | `POST /orders` | Carries the collateral commitment, input proof and viewing key. |
| [Get Order](get-order.mdx) | `GET /orders/{order_id}` | Current status while the order remains in bounded server retention. |
| [Modify Order](modify-order.mdx) | `PUT /orders/{order_id}` | Atomic cancel + replace. |
| [Cancel Order](cancel-order.mdx) | `DELETE /orders/{order_id}` | Signed cancel intent. |

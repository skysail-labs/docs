...
The [order operations](./ws-trading.mdx),
[orders channel](./orders-channel.mdx), and
[fills channel](./fills-channel.mdx) pages document facets of this one session.
...
| ---------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| `op`                   | Yes      | `"login"`.                                                                                                                           |
| `token`                | Yes      | A bearer token from [`POST /auth/token`](../getting-started/authentication.mdx).                                                                  |
...
| -------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| `orders` | Per-account | Settlement reservations, confirmed fills, terminal failures, cancellations, and expiries. Same payloads as the [Orders Channel](./orders-channel.mdx). |
| `fills`  | Per-account | Your continuation-fill memos. Same payloads, and the same verify-before-store rule, as the [Fills Channel](./fills-channel.mdx).                       |
...
[`POST /orders`](../orders/place-order.mdx) and the [order operations](./ws-trading.mdx), so
...
[`PUT /orders/{order_id}`](../orders/modify-order.mdx).
...
reconstructed client-side from the `tree` channel (or the [`/tree/*`](../tree/README.mdx)
...
[`GET /settlement/status/{batch_id}`](../settlement/settlement-status.mdx).

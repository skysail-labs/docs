...
| `unsubscribe` | `request_id?`, `channels` | removes one or more subscriptions |
| `order.place` | `request_id?`, `params` (a full [Place Order](../orders/place-order.mdx) body) | `POST /orders` |
| `order.cancel` | `request_id?`, `order_id`, `params` (`trading_key`, `cancel_nonce`, `session_id`, `trading_key_signature`) | `DELETE /orders/{id}` |
| `order.modify` | `request_id?`, `order_id`, `params` (a [Modify Order](../orders/modify-order.mdx) body) | `PUT /orders/{id}` |
| `ping` | `request_id?` | none |
| `logout` | `request_id?` | closes the session cleanly |
...
| `error` | `seq`, `request_id?`, `code`, `message` | A frame failed. `code` is the stable numeric [error code](../getting-started/error-codes.mdx); `message` is the same reason the REST path would have returned. |
...
```mermaid
flowchart TD
    A[socket opens] --> B["order.place ×N"]
    B --> C[connectivity lost / socket closes]
...
For live order state and fill memos, subscribe to the [Orders Channel](./orders-channel.mdx)
and [Fills Channel](./fills-channel.mdx) on this same socket.

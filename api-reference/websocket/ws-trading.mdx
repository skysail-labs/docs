...
| `error` | `seq`, `request_id?`, `code`, `message` | A frame failed. `code` is the stable numeric [error code](../getting-started/error-codes.md); `message` is the same reason the REST path would have returned. |

...
```mermaid
stateDiagram-v2
    [*] --> pending_settlement: order.place
    pending_settlement --> partially_filled
    partially_filled --> partially_filled: further fills
    partially_filled --> fully_filled
    fully_filled --> [*]
    pending_settlement --> settlement_failed: fresh order required
    settlement_failed --> [*]
    [*] --> resting: order.place, rests
    resting --> expired
    resting --> cancelled
    expired --> [*]
    cancelled --> [*]
...
the [Fills Channel](./fills-channel.mdx).

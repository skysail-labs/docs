...
against [`GET /transport-attestation`](getting-started/transport-and-attestation.mdx)
...
| Page | What it covers |
|---|---|
| [Base URLs](getting-started/base-urls.mdx) | Hosts, health and time. |
| [Authentication](getting-started/authentication.mdx) | Exchanging credentials for a short-lived bearer token. |
| [Transport & Attestation](getting-started/transport-and-attestation.mdx) | Verifying the enclave and its TLS certificate. |
| [Error Codes](getting-started/error-codes.mdx) | The error envelope and every code it can carry. |

## Services

| Group | Endpoints | Purpose |
|---|---|---|
| [Orders](orders/README.mdx) | `POST`/`GET`/`PUT`/`DELETE /orders` | The order lifecycle. |
| [Instruments](instruments/README.mdx) | `/instruments` | Tradable markets, tick sizes, price scales. |
| [Account](account/README.mdx) | `/account`, `/account/settings` | Your open orders and trading settings. |
| [Merkle Tree](tree/README.mdx) | `/tree/*` | Roots, inclusion proofs and leaf pagination. |
| [Settlement](settlement/settlement-status.mdx) | `/settlement/status/{batch_id}` | Per-match outcomes and on-chain signatures. |
| [Transparency](settlement/transparency.mdx) | `/transparency` | Public solvency snapshot and engine identity. |
| [System](system/system-status.mdx) | `/system/status`, `/health`, `/time`, `/info` | Liveness, degraded modes, server clock. |
| [WebSocket](websocket/session-stream.mdx) | `/v1/stream` | Multiplexed orders and fills channels. |

## Conventions

- **Integers, never floats.** Prices and sizes are integers scaled by the
  instrument's `price_scale`. Read it from
  [`GET /instruments`](instruments/list-instruments.mdx) at startup.
- **Bearer tokens are short-lived.** Mint one with
  [`POST /auth/token`](getting-started/authentication.mdx) and refresh it; do not

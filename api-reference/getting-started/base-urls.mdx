...
[Transport & Attestation](./transport-and-attestation.mdx).
...
See [Error Codes](error-codes.mdx) for the full catalogue of conditions
per status.
...
Use it for load-balancer health checks; use [`/system/status`](../system/system-status.mdx)

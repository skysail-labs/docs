...
| `503 Service Unavailable` | Subsystem down, unsafe mirror, or auth at capacity | Matching or settlement is not available (see [`/system/status`](../system/system-status.mdx)); a tree mirror is unsafe (`5002`); or credential verification is momentarily saturated (`1402`). |
